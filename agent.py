...
class Agent:
    def __init__(self, arms: int, stationary: bool = True):
        ...
        self.optimal_action_percentage = 0
        self.set_maximum_reward()
        self.exploration_rate = 0.2
        self.value_estimates = np.full((arms,), 1.)
    ...
    def run(self, rounds: int):
        ...
        for i in range(0, rounds):
            ...
            self.update_averages(reward=reward, regret=regret, pull=i + 1, optimal=optimal)
            self.update_value_estimate(reward=reward, arm=arm, pull=i + 1)
            # self.exploration_rate = 1 / math.log(i + 0.00001)
        print(
            f'Optimal action percentage = {self.optimal_action_percentage:1.3f}, Average reward = {self.average_reward:1.3f}, average regret = {self.average_regret:1.3f}')
    ...
    def update_averages(self, reward: int, regret: int, pull: int, optimal: int):
        ...
        self.optimal_action_percentage = self.optimal_action_percentage + (
                optimal - self.optimal_action_percentage) / pull
